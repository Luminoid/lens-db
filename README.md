...
- [x] Tags / Dots view toggle (tag view is the default): the tag view renders every lens as a labelled chip (none hidden, none overlapping) via a custom 2D packer ([`src/lib/chart/tagLayout.ts`](src/lib/chart/tagLayout.ts)) on an arbitrarily tall page-scrolled SVG canvas; a zoom draws as a horizontal bar spanning its focal range (lane-packed into discrete f-stop rows when Y is aperture, else stacked vertically at its true X), a prime as a short chip; the focal (X) axis floats pinned to the bottom of the viewport as the canvas scrolls ([`src/lib/components/TagAxis.svelte`](src/lib/components/TagAxis.svelte)); dynamic tag text (brand when multi-brand + focal/aperture + series badge + optional spec); URL-persisted (`?view=dots`, `?tag=price`). The Dots view (scatter/segments with pan-zoom) shares the same important-focal-length / f-stop ticks and gridlines.
...
src/
  ...
  lib/components/   # LensChart, FilterPanel, AxisControls, RangeSlider, CompareTray, LangSwitch, ThemeToggle, TagAxis (floating focal axis)
